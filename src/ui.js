...
export function setupMenu(settings,{play,change,preview,equip}){
 ...
 for(const spec of Object.values(BIKES)){
  ...
  const photo=document.createElement('img');photo.src='/bikes/'+spec.id+'.png';photo.alt=spec.name+(spec.id==='rieju'?' — pelimalli':' — tuotekuva');photo.width=800;photo.height=500;photo.decoding='async';
